...
private:
    ...
    // Kernel name:
    const char* const kernel_symbol = "_Z17julia_upchan_407413CuDeviceArrayI5Int32Li1ELi1EES_I9Float16x2Li1ELi1EES_I6Int4x8Li1ELi1EES_IS2_Li1ELi1EES_IS0_Li1ELi1EE";
...
cudaUpchannelizer_U32::cudaUpchannelizer_U32(Config& config,
                                             const std::string& unique_name,
                                             bufferContainer& host_buffers,
                                             cudaDeviceInterface& device) :
    cudaCommand(config, unique_name, host_buffers, device, "Upchannelizer_U32", "Upchannelizer_U32.ptx")
...
// int cudaUpchannelizer_U32::wait_on_precondition(const int gpu_frame_id) {
//     
//     
//     const std::string Tactual_buffer_name = "host_" + Tactual_memname;
//     Buffer* const Tactual_buffer = host_buffers.get_buffer(Tactual_buffer_name.c_str());
...
cudaEvent_t cudaUpchannelizer_U32::execute(const int gpu_frame_id,
    ...
    DEBUG("Running CUDA Upchannelizer_U32 on GPU frame {:d}", gpu_frame_id);
    const CUresult err =
    ...
    CHECK_CUDA_ERROR(cudaMemcpyAsync(host_info[gpu_frame_id].data(),
...
void cudaUpchannelizer_U32::finalize_frame(const int gpu_frame_id) {
    ...
    for (std::size_t i = 0; i < host_info[gpu_frame_id].size(); ++i)
        if (host_info[gpu_frame_id][i] != 0)
            ERROR("cudaUpchannelizer_U32 returned 'info' value {:d} at index {:d} (zero indicates noerror)",
