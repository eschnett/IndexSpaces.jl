...
private:
    ...
    // Kernel name:
    const char* const kernel_symbol = "_Z17julia_upchan_407113CuDeviceArrayI5Int32Li1ELi1EES_I9Float16x2Li1ELi1EES_I6Int4x8Li1ELi1EES_IS2_Li1ELi1EES_IS0_Li1ELi1EE";
...
cudaUpchannelizer_U64::cudaUpchannelizer_U64(Config& config,
                                             const std::string& unique_name,
                                             bufferContainer& host_buffers,
                                             cudaDeviceInterface& device) :
    cudaCommand(config, unique_name, host_buffers, device, "Upchannelizer_U64", "Upchannelizer_U64.ptx")
...
void cudaUpchannelizer_U64::finalize_frame(const int gpu_frame_id) {
    ...
    for (std::size_t i = 0; i < host_info[gpu_frame_id].size(); ++i)
        if (host_info[gpu_frame_id][i] != 0)
            ERROR("cudaUpchannelizer_U64 returned 'info' value {:d} at index {:d} (zero indicates noerror)",
