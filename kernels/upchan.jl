...
function upchan!(emitter)
    ...
    layout_Γ³reim_registers = Layout([
        ...
        [Freq(:freq, 1 << (4 + bit), 2) => Thread(:thread, 1 << [4, 3][bit + 1], 2) for bit in 0:(Ubits - 5)]...,
        [Dish(:dish, 1 << (5 + bit), 2) => Thread(:thread, 1 << [4, 3][bit + 1], 2) for bit in (Ubits - 4):1]...,
    ])
    # eqn. (62)
    push!(
        emitter.statements,
